...
describe("MasterMilker", function () {
  before(async function () {
    ...
    this.MasterMilker = await ethers.getContractFactory("MasterMilker")
  ...
  it("should set correct state variables", async function () {
    this.chef = await this.MasterMilker.deploy(this.milky.address, this.dev.address, "1000", "0", "1000")
    await this.chef.deployed()

    await this.milky.transferOwnership(this.chef.address)

    const milky = await this.chef.milky()
    const devaddr = await this.chef.devaddr()
    const owner = await this.milky.owner()

    expect(milky).to.equal(this.milky.address)
  ...
  context("With ERC/LP token added to the field", function () {
    ...
    it("should give out SUSHIs only after farming time", async function () {
      ...
      expect(await this.milky.balanceOf(this.bob.address)).to.equal("0")
      ...
      expect(await this.milky.balanceOf(this.bob.address)).to.equal("100")

      await advanceBlockTo("104")
      await this.chef.connect(this.bob).deposit(0, "0") // block 105

      expect(await this.milky.balanceOf(this.bob.address)).to.equal("500")
      expect(await this.milky.balanceOf(this.dev.address)).to.equal("50")
      expect(await this.milky.totalSupply()).to.equal("550")
    })
    ...
    it("should not distribute SUSHIs if no one deposit", async function () {
      ...
      expect(await this.milky.totalSupply()).to.equal("0")
      expect(await this.milky.balanceOf(this.bob.address)).to.equal("0")
      expect(await this.milky.balanceOf(this.dev.address)).to.equal("0")
      ...
      expect(await this.milky.totalSupply()).to.equal("1100")
      expect(await this.milky.balanceOf(this.bob.address)).to.equal("1000")
      expect(await this.milky.balanceOf(this.dev.address)).to.equal("100")
      expect(await this.lp.balanceOf(this.bob.address)).to.equal("1000")
    })
    ...
    it("should distribute SUSHIs properly for each staker", async function () {
      ...
      //   MasterMilker should have the remaining: 10000 - 5666 = 4334
      await advanceBlockTo("319")
      await this.chef.connect(this.alice).deposit(0, "10", { from: this.alice.address })
      expect(await this.milky.totalSupply()).to.equal("1100")
      expect(await this.milky.balanceOf(this.alice.address)).to.equal("566")
      expect(await this.milky.balanceOf(this.bob.address)).to.equal("0")
      expect(await this.milky.balanceOf(this.carol.address)).to.equal("0")
      expect(await this.milky.balanceOf(this.chef.address)).to.equal("434")
      expect(await this.milky.balanceOf(this.dev.address)).to.equal("100")
      // Bob withdraws 5 LPs at block 330. At this point:
      //   Bob should have: 4*2/3*1000 + 2*2/6*1000 + 10*2/7*1000 = 6190
      await advanceBlockTo("329")
      await this.chef.connect(this.bob).withdraw(0, "5", { from: this.bob.address })
      expect(await this.milky.totalSupply()).to.equal("2200")
      expect(await this.milky.balanceOf(this.alice.address)).to.equal("566")
      expect(await this.milky.balanceOf(this.bob.address)).to.equal("619")
      expect(await this.milky.balanceOf(this.carol.address)).to.equal("0")
      expect(await this.milky.balanceOf(this.chef.address)).to.equal("815")
      expect(await this.milky.balanceOf(this.dev.address)).to.equal("200")
      ...
      expect(await this.milky.totalSupply()).to.equal("5500")
      expect(await this.milky.balanceOf(this.dev.address)).to.equal("500")
      // Alice should have: 5666 + 10*2/7*1000 + 10*2/6.5*1000 = 11600
      expect(await this.milky.balanceOf(this.alice.address)).to.equal("1159")
      // Bob should have: 6190 + 10*1.5/6.5 * 1000 + 10*1.5/4.5*1000 = 11831
      expect(await this.milky.balanceOf(this.bob.address)).to.equal("1183")
      // Carol should have: 2*3/6*1000 + 10*3/7*1000 + 10*3/6.5*1000 + 10*3/4.5*1000 + 10*1000 = 26568
      expect(await this.milky.balanceOf(this.carol.address)).to.equal("2657")
    ...
    it("should give proper SUSHIs allocation to each pool", async function () {
      ...
      this.chef = await this.MasterMilker.deploy(this.milky.address, this.dev.address, "100", "400", "1000")
      await this.milky.transferOwnership(this.chef.address)
      ...
      expect(await this.chef.pendingMilky(0, this.alice.address)).to.equal("1166")
      await advanceBlockTo("430")
      // At block 430. Bob should get 5*2/3*1000 = 3333. Alice should get ~1666 more.
      expect(await this.chef.pendingMilky(0, this.alice.address)).to.equal("1333")
      expect(await this.chef.pendingMilky(1, this.bob.address)).to.equal("333")
    })
    ...
    it("should stop giving bonus SUSHIs after the bonus period ends", async function () {
      ...
      expect(await this.chef.pendingMilky(0, this.alice.address)).to.equal("1500")
      // At block 606, Alice withdraws all pending rewards and should get 10600.
      await this.chef.connect(this.alice).deposit(0, "0", { from: this.alice.address })
      expect(await this.chef.pendingMilky(0, this.alice.address)).to.equal("0")
      expect(await this.milky.balanceOf(this.alice.address)).to.equal("1600")
    })
  })
})
